...
class AuthController with ChangeNotifier {
  ...
  Future<void> sendVerificationCode(ConfigModel config, SignUpModel signUpModel, {String? type, FromPage? fromPage}) async {
    ...
    notifyListeners();
    print(config.customerVerification!.status);
    print(type);
    print(config.customerVerification?.firebase);
    print(config.customerVerification?.phone);
  ...
  Future<void> reSendVerificationCode(ConfigModel config, SignUpModel signUpModel, {String? type, FromPage? fromPage}) async {
    _resendButtonLoading = true;
    ...
    showCustomSnackBar("Otp send successfully", Get.context!);
    _resendButtonLoading = false;
    notifyListeners();
  }
  ...
  Future<ResponseModel> registerWithOtp (String name, {String? email, required String phone, required String referCode}) async{
    _isPhoneNumberVerificationButtonLoading = true;
    _loginErrorMessage = '';
    notifyListeners();
    ApiResponseModel apiResponse = await authServiceInterface.registerWithOtp(name, email: email, phone: phone,referCode: referCode);
    ResponseModel responseModel;
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      String? token;
      Map map = apiResponse.response!.data;
  ...
  Future<(ResponseModel?, String?)> verifyPhoneForOtp(String phone) async {
    ...
    if (apiResponse.response != null && apiResponse.response!.statusCode == 200) {
      ...
      if(token != null){
        await authServiceInterface.saveUserToken(token);
        responseModel = ResponseModel('verification', map["isUserExist"]);
      }else if(tempToken != null){
        responseModel = ResponseModel( 'verification', map["isUserExist"]);
      }
    } else {
