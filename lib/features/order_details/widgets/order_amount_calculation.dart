...
class OrderAmountCalculation extends StatelessWidget {
  ...
  final double referDiscount;
  final double brillDaddyDiscount;
  final OrderDetailsController orderProvider;
  const OrderAmountCalculation({super.key, required this.itemTotalAmount, required this.discount, this.eeDiscount, required this.tax, required this.shippingCost, required this.orderProvider, required this.referDiscount, required this.brillDaddyDiscount});
    ...
    return  Container(padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          AmountWidget(title: "Referral Discount",
            amount: PriceConverter.convertPrice(context, referDiscount),),

          AmountWidget(title: "Wallet Utilization",
            amount: PriceConverter.convertPrice(context, brillDaddyDiscount),),
          ...
          AmountWidget(title: getTranslated('total_payable', context),
            amount: PriceConverter.convertPrice(context, (itemTotalAmount + shippingCost - eeDiscount! - orderProvider.orders!.discountAmount! - discount  + tax) - referDiscount - brillDaddyDiscount),),
